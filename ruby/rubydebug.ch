...
=== cont
  (rdb:1) c[ont][ nnn]
  (run until program ends or hits breakpoint or reaches line nnn)
...
=== eval
  (rdb:1) e[val] expression evaluate expression and print its value,
                 alias for p. To turn on autoeval, use 'set autoeval'
...
=== frame
  (rdb:1) f[rame] frame-number
    ...
    So 'frame -1' moves to the oldest frame,
    and 'frame 0' moves to the newest frame.
...
=== ps
  (rdb: 1) ps array - print array in columnized sorted order
  (e.g. "ps Kernel.methods")
...
=== restart
  (rdb:1) restart|R [args] Restart the program.
                    This is is a re-exec - all debugger state is lost.
